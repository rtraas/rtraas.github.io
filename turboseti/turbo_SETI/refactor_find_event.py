...
def make_table(filename,init=False):
    ...
    if init:
        ...
        headers = []
        H_filtered = []
        H = [hit.strip().split('\t') for hit in hits]
        for h in H:
            if h[0][0] == '#':
                headers.append(h)
            else:
        ...
        print("G[4701][1] = ", G[4701][1])
        dfs = []
        print("len(G) = ", len(G))
        print("len(hits[9:]) = ", len(hits[9:]))
        ...
        #index_one = []
        #for df in G:
        #    index_one.append(df[1])
        #print(index_one)

        all_hits = [hit.strip().split('\t') for hit in hits[9:]]
        print("all_hits =", all_hits)
        print("*all_hits = ", list(zip(*all_hits)))
        #print("len(*all_hits) = ", len(list(zip(*all_hits))))
        print("len(all_hits) = ", len(list(zip(all_hits))))
        #print("all_hits zipped",list(zip(all_hits)))

        #Uses "unpacking asterisk (*)"
        #print("list(zip(*all_hits))) = ", list(zip(*all_hits)))

        #print("len(list(zip(*all_hits)))) = ", len(list(zip(*all_hits))))

        #y = [df for df in list(zip(*all_hits))[0]]
        #print("y = ", y)
        #for yy in y:
            #if type(yy) != str:
                #print(y.index(yy))
        ...
        if all_hits:
            ...
            print("TopHitNum = ", TopHitNum)
            #SWITCH INDEX TO "0" FOR BELOW
            #DriftRate = [float(df) for df in list(zip(*all_hits))[1]]
            #DriftRate = [float(df) for df in list(zip(*all_hits))[0]]
            DriftRate = []
            for df in G:
                DriftRate.append(df[1])
            print("DriftRate = ", DriftRate)
...
def find_events(dat_file_list,
                ...
                filter_threshold,
                on_off_first,
                on_source_complex_cadence, complex_cadence):
    ...
    for i,dat_file in enumerate(dat_file_list):
        ...
        #if off
        if (i%2 == odd_even_indicator and on_source_complex_cadence == False) or (on_off_indicator == 0 and on_source_complex_cadence != False):
            ...
            #Using refactor_make_table function
            off_table_i=unzip.refactor_make_table(dat_file)
            ...
            #Using refactor_make_table function
            on_table_i=unzip.refactor_make_table(dat_file)
            #on_table_i['status'] = 'on_table_%i'%on_count
            on_table_i['status'] = str("on_table_" + str(on_count))
